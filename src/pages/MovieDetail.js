...
export  function MovieDetail() {
  ...
  useEffect(()=>{
    async function FetchMovieDetail(){
      const response = await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=fe000dff045e0e669c59ef891e06c222`)
      ...
      <section className="flex justify-around py-5 flex-wrap">
        ...
        <div className="max-w-2xl text-grey-700 dark:text-white text-lg">
          ...
          {movie.genres ? (<p className="my-7 flex flex-wrap gap-2">
            {
              movie.genres.map((genre)=>(
                <span className="mr-2 border boder-grey-200 rounded p-2 dark:border-grey-600" key={genre.id}>{genre.name}</span>
              ))
          ...
          <div className="flex items-center">
            ...
            <p className="my-4">
              <span className="mr-2 font-bold">Runtimes:</span>
